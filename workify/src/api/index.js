...
//user api
export const getUsers = async (token) => await API.get('/users/find', { headers: { "Authorization" : `Bearer ${token}` }},{withCredentials: true});
...
//teams api
export const createTeam = async (team,token) => await API.post('/team/createTeam', team,{ headers: { "Authorization" : `Bearer ${token}`}})
export const getTeams = async (id,token) => await API.get(`/team/getTeam/${id}`,{ headers: { "Authorization" : `Bearer ${token}` }},{ withCredentials: true });
